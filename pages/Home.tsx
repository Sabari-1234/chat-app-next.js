    ...
    const items =
      Members && Members.length > 1
        ? Members.filter(
          ...
          ).map((member: any, id: any) => ({
            title: member.name,
            description:
              "Embark on exciting journeys and thrilling discoveries.",
            header: <Skeleton profile={member.image} />,
            email: member.email,
          }))
        : [];
    return (
...
const Skeleton = ({ profile }: any) => (
  <div className="flex flex-1 w-full h-full min-h-[6rem] rounded-xl bg-gradient-to-br  justify-center">
    <img
      src={profile}
      alt="profile image not shown"
